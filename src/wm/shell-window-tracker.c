/*
 * This file inherit from gnome-shell/src/shell-window-tracker.c
 */
...
update_focus_app (ShellWindowTracker *self)
{
    ...
    new_focus_win = meta_display_get_focus_window(elsa_wm_plugin_get_display(
        elsa_wm_plugin_get()));
...
load_initial_windows (ShellWindowTracker *tracker)
{
    ...
    MetaScreen *screen = elsa_wm_plugin_get_screen(elsa_wm_plugin_get());
...
shell_window_tracker_init (ShellWindowTracker *self)
{
    ...
    self->launched_pid_to_app = g_hash_table_new_full(NULL, NULL, NULL, 
                                              (GDestroyNotify) g_object_unref);
